      ...
      */}
      <div style={{ border: "1px solid #cccccc", padding: 16, margin: "auto", marginTop: 64 }}>
        ...
        <div style={{ margin: 8 }}>
          ...
          <Input
            onChange={e => {
              setCompCategory(e.target.value);
            }}
          />
          <Button
            style={{ marginTop: 8 }}
            onClick={async () => {
              const result = tx(writeContracts.CompetitionFactory.createCompetition(compName, compDesc, compCategory), update => {
              ...
              let r = await (await result).wait();
              console.log('*****await result', r);
              let ev = r.events.find(x => x.event === 'CompetitionCreated');
              let compAddress = ev.args[0];
